...
- Mermaid: **ครอบข้อความและชื่อ participant ด้วย double quote ทุกจุด** ห้ามใส่ `;` ท้ายบรรทัด — **ยกเว้น `stateDiagram-v2`** ที่ห้ามครอบชื่อ state ด้วย quote ตรงในเส้น transition (parse ไม่ผ่าน) ต้องประกาศ `state "ข้อความไทย" as shortId` ก่อน แล้วเขียน transition ด้วย shortId ที่ไม่มี quote (label บนเส้นยังครอบ quote ได้ปกติ)
