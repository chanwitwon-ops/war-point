...
- **หน้าที่:** ปิดใช้งาน `PET` ตัวปัจจุบัน (กลายเป็นประวัติ), สร้าง `PET` ใหม่ที่ขั้นไข่, คืนแต้มที่ใช้ได้กลับมาเท่ากับยอดสะสมรวม
...
- **กฎทางธุรกิจที่ endpoint นี้บังคับ:** หลังรีเซ็ต `available_points = total_earned_points` เสมอ (spec 02 ข้อ ข.3); ตัว `PET` เก่าต้อง `is_active=false` ไม่ใช่ถูกลบ (กลายเป็นรายการในอัลบั้ม FE-72/73 อัตโนมัติ); **ขอบเขตว่าการคืนแต้มนี้รวมแต้มที่เคยใช้ซื้อไอเทมช่วย (API-41) ด้วยหรือไม่ยังไม่ล็อก** (ดู Gap — ช่องโหว่ "รีเซ็ตแล้วได้แต้มค่าไอเทมคืนวนซ้ำ") — ต้อง implement ตามการตัดสินใจที่ปิด Gap นี้ก่อน ห้ามเดาเอง
...
- **G-API-05:** ช่องโหว่ "รีเซ็ตแล้วได้แต้มค่าไอเทมคืนวนซ้ำ" ยังไม่ปิด — กระทบ business logic ของ API-38 (reset) โดยตรงว่าต้องยกเว้น `entry_type='item_purchase'` ออกจากยอดที่คืนหรือไม่ เอกสารนี้ระบุพฤติกรรมที่ต้องรอการตัดสินใจไว้ชัดเจนแล้วในรายละเอียด endpoint แทนการเดา
