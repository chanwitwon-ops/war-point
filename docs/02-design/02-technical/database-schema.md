...
| id | รหัสอ้างอิง | PK | ไม่ | - | - |
| student_id | รหัสอ้างอิง | FK → STUDENT | ไม่ | - | - |
| species_id | รหัสอ้างอิง | FK → SPECIES | ไม่ | - | เลือกตอนซื้อไข่ (FE-44) เปลี่ยนไม่ได้หลังจากนั้น (เปลี่ยนสาย = รีเซ็ตสร้างแถวใหม่) |
...
| is_active | บูลีน | - | ไม่ | true | false = ถูกแทนที่ด้วยการรีเซ็ต/เปลี่ยนสายพันธุ์ (FE-26) — คือรายการในอัลบั้มประวัติ |
...
- **กฎความถูกต้องระดับข้อมูล:** CHECK `stat_total IN (50,100,150,200,250)`; การรีเซ็ต (FE-26) ต้องเป็น transaction เดียว: (1) UPDATE แถวเดิม `is_active=false, disabled_at=now()`, (2) INSERT แถวใหม่ `stage='egg', stat_total=50`, (3) INSERT `POINTS_LEDGER_ENTRY` ชนิด `pet_reset_refund` ให้ `available_points` กลับมาเท่ากับ `total_earned_points` — **ขอบเขตว่าการคืนแต้มนี้รวมแต้มที่เคยใช้ซื้อไอเทม (STUDENT_ITEM_INVENTORY) ด้วยหรือไม่ยังไม่ล็อก (ดู Gap ช่องโหว่ FE-26/FE-78)**
...
- **กฎความถูกต้องระดับข้อมูล:** CHECK `quantity >= 0`; การซื้อต้องอยู่ใน transaction เดียวกับการหัก `POINTS_ACCOUNT.available_points` และ insert `POINTS_LEDGER_ENTRY(entry_type='item_purchase')`
...
- **Soft delete เป็นค่าเริ่มต้นทุกตาราง** (ดูข้อ 0) — การ "ลบ" ในระบบนี้ทั้งหมดคือการตั้ง `is_active=false` เท่านั้น ไม่มีคำสั่งลบถาวรที่ผู้ใช้เข้าถึงได้
  ...
  - `PET`: ปิดใช้งานเมื่อรีเซ็ต/เปลี่ยนสายพันธุ์ (FE-26) — แถวเก่ากลายเป็นรายการในอัลบั้มบันทึกเส้นทางการเติบโตโดยอัตโนมัติ (FE-72/FE-73) **ไม่มีทางลบแถว PET ได้เลยในระบบนี้**
...
- **G-DB-07 (สืบทอดจาก spec 06 Open Question ข้อ 6 / architecture.md ข้อ 9):** ช่องโหว่ "รีเซ็ตแล้วได้แต้มค่าไอเทมคืนวนซ้ำ" ยังไม่ปิด — กระทบ business logic ของ transaction รีเซ็ตใน `PET` โดยตรงว่าการคืนแต้ม (`pet_reset_refund`) ควรคำนวณจากผลรวมของ `POINTS_LEDGER_ENTRY` ชนิด `stage_unlock`+`egg_purchase` เท่านั้น หรือรวม `item_purchase` ด้วย — เอกสารนี้ไม่ได้เลือกฝั่งใดฝั่งหนึ่งแทนผู้ใช้
