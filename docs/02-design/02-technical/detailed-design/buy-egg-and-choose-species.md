...
| ERR-04 | `available_points < 1` | ไม่หักแต้ม ไม่สร้าง PET | 422 INSUFFICIENT_POINTS |
...
```mermaid
stateDiagram-v2
    [*] --> "ไม่มี PET ที่ใช้งานอยู่"
    "ไม่มี PET ที่ใช้งานอยู่" --> "PET(stage=egg, is_active=true)" : "ซื้อไข่สำเร็จ (DD-01)"
```
